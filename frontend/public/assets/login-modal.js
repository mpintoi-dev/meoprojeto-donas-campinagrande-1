...
(function () {
  ...
  function injectStyles() {
    ...
    s.textContent = ''
      ...
      + '#'+MODAL_ID+'{background:#fff;border-radius:6px;width:100%;max-width:340px;position:relative;'
      ...
      + '#'+MODAL_ID+' .lm-body{padding:22px 26px 18px;display:block}'
      + '#'+MODAL_ID+' .lm-form{display:grid;grid-template-columns:auto 1fr;gap:12px 12px;align-items:center}'
      ...
      + '#'+MODAL_ID+' .lm-links{margin:16px 0 0;padding:0;list-style:none}'
      + '#'+MODAL_ID+' .lm-links li{position:relative;padding-left:14px;margin:6px 0}'
      + '#'+MODAL_ID+' .lm-links li::before{content:"•";position:absolute;left:0;color:#194a8a;font-weight:700}'
      ...
      + '#'+MODAL_ID+' .lm-error{color:#c40000;font-size:13px;background:#fff3f3;border:1px solid #f3c2c2;'
      + 'padding:8px 10px;border-radius:4px;margin-top:12px;display:none}'
      + '#'+MODAL_ID+' .lm-error.show{display:block}'
      + '#'+MODAL_ID+' .lm-footer{padding:16px 26px 24px;text-align:center}'
  ...
  function buildModal() {
    ...
    back.innerHTML = ''
      ...
      + '    </div>'
      + '    <ul class="lm-links" style="padding:14px 0 0 4px;margin:0">'
      + '      <li><a href="#" data-testid="link-cadastro" id="lm-not-registered">Realizar cadastro do candidato</a></li>'
